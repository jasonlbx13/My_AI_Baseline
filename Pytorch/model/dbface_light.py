...
class UpModule(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=2, stride=2,  bias=False, mode="UCBA"):
        ...
        if self.mode == "UCBA":
            ...
            self.conv = DSC(3, in_channels, out_channels, nn.ReLU(inplace=True), 1)
...
class ContextModule(nn.Module):
    def __init__(self, in_channels):
        ...
        self.inconv = DSC(3, in_channels, block_wide, nn.ReLU(inplace=True), stride=1)
        self.upconv = DSC(3, block_wide, block_wide, nn.ReLU(inplace=True), stride=1)
        self.downconv = DSC(3, block_wide, block_wide, nn.ReLU(inplace=True), stride=1)
        self.downconv2 = DSC(3, block_wide, block_wide, nn.ReLU(inplace=True), stride=1)
...
class DetectModule(nn.Module):
    def __init__(self, in_channels):
        ...
        self.upconv = DSC(3, in_channels, in_channels // 2, nn.ReLU(inplace=True), stride=1)
